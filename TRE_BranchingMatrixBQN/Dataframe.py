...
class Dataframe:
    ...
    def init_df_prices(self):
        ...
        df_bids = df_bids.groupby(["datetime_start_utc"], as_index=False).apply(lambda grp: grp.assign(median_price=grp[grp['volume_activated'] > 0]['price'].median()))
        ...
        df_bids = utils.add_column_and_merge(df_bids, 'median_price')

        # add weekday into df
        # cast to datetime for merge 
        df_bids['datetime_start_utc'] = pd.to_datetime(df_bids['datetime_start_utc'], utc=True)
        ...
        day_ahead_prices = pd.read_csv('Data/day_ahead_prices_2021.csv', sep=";")
        # if read from DB
        #day_ahead_prices.drop('Price_area', axis=1, inplace=True)
        day_ahead_prices['datetime_start_utc'] = pd.to_datetime(day_ahead_prices['datetime_start_utc'], utc = True)
        ...
        # CAST to DAte time UTC
        df_forecast['datetime'] = pd.to_datetime(df_forecast['datetime'], utc = True)
        ...
        df_bids_prices_forecast = df_bids_prices_forecast[[
            'spot_price_actual', 'estimated_max_price', 'estimated_min_price','estimated_mean_price','estimated_median_price', 'estimated_sum_volume_activated', 'estimated_std', 
            'estimated_count_successfull_bids', 'weekday', 'is_working_day', 'hour',
            'max_price', 'min_price','mean_price','sum_volume_activated', 'std', 'count_successfull_bids', 'volume', 'datetime_start_utc',
            'forecast']]
    ...
    def init_df_activated_bids(self):
        ...
        # filter for up direction only and successfull bids
        df_bids = df_bids[df_bids['AUCTION_MRID'] =='TREnergie+_s']
        df_bids = df_bids[df_bids['volume_activated'] >  0]
        #df_bids.to_csv("Data/All_activated_bids_2022.csv", sep=";")
        return df_bids